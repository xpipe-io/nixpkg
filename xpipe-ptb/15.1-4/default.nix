...
let
  ...
  hash = {
    x86_64-linux = "sha256-0o4j/Obrc9zbr8y84zZw/AzyzwQW+UtzzyfljNEkaYk=";
    aarch64-linux = "";
  }.${system} or throwSystem;
...
in stdenvNoCC.mkDerivation rec {
  ...
  installPhase = ''
    ...
    makeShellWrapper "$out/opt/$pkg/bin/xpiped_raw" "$out/opt/$pkg/bin/xpiped" \
      --prefix LD_LIBRARY_PATH : "${lib.makeLibraryPath [ fontconfig gtk3 udev util-linux libsecret socat ]}"
